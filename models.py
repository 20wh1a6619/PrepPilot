...
def extract_skills_from_jd(job_description):
        ...
        "messages": [
            {
                ...
                "content": (
                    ...
                    "Use standard short names (e.g., Python, AWS, Docker). "
                    "Do not include explanations or duplicates."
                )
            },
            {
                "role": "user",
    ...
    try:
        ...
        # Deduplicate + Title Case + Sort (match old format)
        final = sorted(set(skill.title() for skill in skills))
